...
import { workloads } from "./AreasTrabajo.js";
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const globalSources = [
                ...
                { id: 'source-2', name: 'Supermicro RMA Policy', url: 'https://www.supermicro.com/en/support/rma_policy' },
            ...
            let currentSelectedTier = null; // To keep track of the active tier
            ...
            tierButtonsContainer.addEventListener('click', function(e) {
                if (e.target.classList.contains('tier-btn')) {
                    currentSelectedTier = e.target.dataset.tier;

                    document.querySelectorAll('.tier-btn').forEach(btn => btn.classList.remove('active'));
                    e.target.classList.add('active');

                    // Clear previous workload buttons and recommendations
                    workloadButtonsContainer.innerHTML = '';
                    recommendationOutput.innerHTML = '<p class="text-center text-stone-500 italic">Seleccione una gama y una carga de trabajo para ver las recomendaciones.</p>';
                    
                    // Show workload section
                    workloadSection.style.display = 'block';

                    // Populate workload buttons based on selected tier
                    workloads.forEach(workload => {
                        if (workload.servers[currentSelectedTier] && workload.servers[currentSelectedTier].length > 0) {
            ...
            workloadButtonsContainer.addEventListener('click', function(e) {
                if (e.target.classList.contains('workload-btn')) {
                    ...
                    if (selectedWorkload && currentSelectedTier) {
                        let outputHtml = '<div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">';
                        const serverIdsForTier = selectedWorkload.servers[currentSelectedTier];
                        ...
                        if (serverIdsForTier && serverIdsForTier.length > 0) {
                            serverIdsForTier.forEach(serverId => {
                                ...
                                if (server) {
                                    ...
                                    if (server.models && server.models.length > 0) {
                                        ...
                                        outputHtml += `<p class="text-sm text-stone-500 italic mt-4">No se encontraron modelos específicos de gama media para esta categoría.</p>`;
                                    }
                                    outputHtml += `
                                            </div>
                                        </div>
                            ...
                            outputHtml += '<p class="text-center text-stone-500 italic col-span-full">No se encontraron recomendaciones de servidores para esta combinación de gama y carga de trabajo.</p>';
                        }
                        outputHtml += '</div>';
